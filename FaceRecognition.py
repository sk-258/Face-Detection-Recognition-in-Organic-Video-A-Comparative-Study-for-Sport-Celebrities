...
from keras.applications import ResNet50  # ✅ UPDATED IMPORT
import pickle
from sklearn.model_selection import train_test_split

# --- Main window ---
main = tkinter.Tk()
main.title("Face Detection & Recognition")
main.geometry("800x700")

global filename
global model
names = ['Gia Mantegna', 'Nicole Gale Anderson', 'Samantha Barks', 'Sarah Hyland', 'Alexis Knapp', 'Alia Shawkat', 
         'Carly Schroeder', 'Diego Boneta', 'Ella Rae Peck', 'Emma Watson', 'Jane Levy', 'Jennifer Lawrence', 
         'Kat Graham', 'Kristen Stewart', 'Liam Hemsworth', 'Shenae Grimes', 'Aaron Johnson', 'Adelaide Clemens', 
         'Anton Yelchin', 'Claire Holt', 'Haley Joel Osment', 'Leah Pipes', 'Luke Kleintank', 'Mackenzie Rosman', 
         'Matthew Lewis', 'Nikki Blonsky', 'Scout Taylor-Compton', 'Alex D. Linz', 'Allison Williams', 
         'AnnaLynne McCord', 'Daniel Radcliffe', 'Hayden Panettiere', 'Hunter Parrish', 'Jesse McCartney', 
         'Keegan Allen', 'Kirsten Prout', 'Michael B. Jordan', 'Rosie Huntington-Whiteley', 'Steven R. McQueen', 
         'Tori Black', 'Charlie McDermott', 'Chris Brown', 'Cody Horn', 'Cody Linley', 'Dakota Johnson', 
         'Danielle Panabaker']
...
def processDataset():
    ...
    textarea.insert(END, "80% images are used to train ResNet50 : "+str(X_train.shape[0])+"\n")
    textarea.insert(END, "20% images are used to test ResNet50 : "+str(X_test.shape[0])+"\n")        
...
def graph():
    ...
    plt.legend(['ResNet50 Accuracy', 'ResNet50 Loss'], loc='upper left')
    plt.title('ResNet50 Training Accuracy & Loss Graph')
    plt.show()    
...
predictButton = Button(main, text="Detect & Recognize Face using Test Images", command=faceRecognition)
